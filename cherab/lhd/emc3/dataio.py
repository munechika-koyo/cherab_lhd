...
DEFAULT_DATA_REPOSITORY = Path(__file__).parent.resolve() / "data"


class DataLoader:
    """EMC3-EIRENE data loader This class offers methods to read raw data
    generated by EMC3-EIRENE.
    ...
    def __init__(self, path: Path | str = DEFAULT_DATA_REPOSITORY) -> None:
        ...
        self.num_total, self.num_plasma, self.num_plasma_vac = _num_cells(self.path / "grid-360")
    ...
    def plasma_radiation(self) -> NDArray[np.float64]:
        ...
        """
        with open(self.path / "RADIATION_1", "r") as f:
    ...
    def impurity_radiation(self) -> NDArray[np.float64]:
        ...
        """
        with open(self.path / "IMP_RADIATION", "r") as f:
            radiation = f.read()
        # extract values
        radiation = re.findall(self.pattern_value, radiation)
        radiation = np.asarray_chkfinite(radiation, dtype=np.float64) * 1.0e6  # [W/cm^3] -> [W/m^3]
    ...
    def radiation(self) -> NDArray[np.float64]:
        """Load total radation data. which means plasma + impurity radiation.
    ...
    def density_electron(self) -> NDArray[np.float64]:
        ...
        """
        with open(self.path / "DENSITY", "r") as f:
            density = f.read()
    ...
    def temperature_electron_ion(self) -> tuple[NDArray[np.float64], NDArray[np.float64]]:
        ...
        """
        with open(self.path / "TE_TI", "r") as f:
            temp = f.read().split()
...
def _num_cells(path: Path) -> tuple[int, int, int]:
    ...
    """
    with open(path / "CELL_GEO", "r") as f:
        try:
